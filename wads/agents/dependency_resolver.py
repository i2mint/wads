...
def scan_project_imports(project_path: Path, exclude_patterns: List[str] = None) -> Dict[str, Set[str]]:
    """
    ...
    if exclude_patterns is None:
        exclude_patterns = ['venv', '.venv', '__pycache__', '.git', 'build', 'dist', '.eggs']
...
def analyze_dependencies(
    ...
    for import_name in all_imports:
        ...
        if not is_declared:
            ...
            using_files = [
                f for f, imports in imports_by_file.items()
    ...
    if check_unused and pyproject_path.exists():
        for dep_pkg in all_declared:
            ...
            if not any(imp in all_imports for imp in potential_imports):
                ...
                reverse_mapped = [k for k, v in IMPORT_TO_PACKAGE.items() if v.lower() == dep_pkg]
    ...
    if missing_packages:
        ...
        for issue in missing_packages[:10]:  # Show first 10
            if issue.is_installed:
                ...
                recommendations.append(
                    f"  - {issue.package_name} (needs installation)"
                )
