...
def parse_pytest_failures(logs: str) -> List[TestFailure]:
    ...
    for match in failure_pattern.finditer(logs):
        ...
        # Look for context around the failure
...
def diagnose_missing_system_deps(failures: List[TestFailure], logs: str) -> List[str]:
    ...
    patterns = {
        ...
        'msodbcsql17': [
            r"ODBC Driver 17 for SQL Server.*not found"
        ],
        'msodbcsql18': [
            r"ODBC Driver 18 for SQL Server.*not found"
        ],
        'ffmpeg': [
            r"ffmpeg.*not found",
            r"libav.*not found"
        ...
        'libsndfile': [
            r"libsndfile.*not found",
            r"sndfile.*not found"
        ],
        'portaudio': [
            r"portaudio.*not found",
            r"libportaudio.*not found"
        ]
    }
...
def diagnose_ci_failure(repo: str, run_id: Optional[int] = None) -> CIDiagnosis:
    ...
    if missing_system_deps:
        config_issues.append(f"Missing system dependencies: {', '.join(missing_system_deps)}")
    ...
    if missing_python_deps:
        config_issues.append(f"Missing Python dependencies: {', '.join(missing_python_deps)}")
