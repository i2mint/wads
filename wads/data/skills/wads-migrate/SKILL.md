...
`ci-to-stub` and `fleet-stub` read `[tool.wads.ci.trigger]`, so a refresh sweep never reverts an on-demand repo. To go back: set `mode = "auto"`, then `wads-migrate ci-to-stub`.
