...
- **On-demand trigger**: when `[tool.wads.ci.trigger].mode = "on-demand"` (the stub has `workflow_dispatch` and an `if:` naming the run-ci marker), nothing runs unless the commit *subject* carries `run_ci_marker` (default `[run ci]`) or someone dispatches the workflow. So "no recent runs", a latest default-branch commit with no run, and pyproject ahead of PyPI are **expected**, not findings: judge check A on the latest run that exists, report the mode in check B, and point at `wads ci-local` (or `gh workflow run ci.yml`) for a fresh signal. A run where only `setup` ran and every other job was skipped is a marker quoted in a PR body, not a failure. If pyproject says on-demand but the stub has no `if:` (rendered before the flip), jobs are still gated but every push spends a setup job: re-render with `wads-migrate ci-to-stub` (**wads-migrate** skill). Likewise if pyproject's `run_ci_marker` differs from the marker in the stub's `if:`, the stub still pre-filters on the old one, so commits using the new marker silently run nothing: same fix. A manual dispatch on the default branch releases, so a release that "nobody pushed" may be one.
