...
def _audit(repo, *, network=True):
    """Run the full read-only audit and return the populated :class:`Report`."""
    ...
    rep.facts["network_checks"] = network
    if network:
...
def audit_repo(repo=".", *, network=True):
    ...
    """
    return _audit(repo, network=network).to_dict()
...
def main(argv=None):
    ...
    rep = _audit(repo, network=not args.no_network)
    if args.json:
