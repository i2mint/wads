...
class TestWorkflowTemplate:
    ...
    def test_setup_job_uses_read_ci_config_action(self, template_data):
        ...
        config_step = next(
            (s for s in steps if s.get('id') == 'config'),
            None
        )
    ...
    def test_publish_job_has_version_and_build_steps(self, template_data):
        ...
        assert any('pypi' in name.lower() or 'publish' in name.lower() for name in step_names)
    ...
    def test_checkout_actions_have_correct_settings(self, template_data):
        ...
        checkout_step = next(
            (s for s in publish_job['steps'] if 'actions/checkout' in s.get('uses', '')),
            None
        )
    ...
    def test_python_setup_actions_use_matrix(self, template_data):
        ...
        python_step = next(
            (s for s in validation_job['steps'] 
