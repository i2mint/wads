...
class TestBuild:
    ...
    Building requires the ``build`` tool, which ships in the ``create`` extra
    (``pip install wads[create]``), not the light core. Skip these tests when it
    is absent so a light install / light CI run stays green; they run whenever
    the ``create`` extra is installed.
    """

    def test_generated_package_can_be_built(self):
        """Test that a package generated by populate can be built."""
        pytest.importorskip("build", reason="requires wads[create]")
        with tempfile.TemporaryDirectory() as tmpdir:
