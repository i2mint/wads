...
def _normalize_package_name(package_name: str) -> str:
    ...
    for suffix in ['-dev', '-devel', '-headers', 'lib', 'lib64']:
...
def _scan_python_imports(project_root: Path) -> set[str]:
    ...
    for py_file in project_root.rglob('*.py'):
        if any(part in py_file.parts for part in ['venv', '.venv', 'site-packages', '__pycache__']):
            continue
    ...
    *,
    ...
    if ci_file:
        ...
        if ci_content:
            ...
            result['python_versions'] = _extract_python_versions(ci_content) or result['python_versions']
            ...
            print(f"  → Testing Python versions: {', '.join(result['python_versions'])}")
    else:
    ...
    if analyze_code:
        ...
        if result['inferred_deps']:
            print(f"  → Inferred {len(result['inferred_deps'])} potential dependencies:")
...
def generate_migration_prompt(analysis: dict) -> str:
    ...
    if analysis['python_versions']:
        prompt_parts.append(f"**Python Versions:** {', '.join(analysis['python_versions'])}")
        prompt_parts.append("")
...
def run_migration_agent(
    ...
    *,
    ...
    if code_blocks:
        ...
        (project_root / 'migration_response.txt').write_text(response_text, encoding='utf-8')
